...
cases = [1, 2]
case_data = {}  # store results per case
for case_num in cases:
    ...
    last_s = None
    x_list, y_list = [], []
    s_list, e_list, dphi_list, V_list = [], [], [], []
    delta_list, torque_list = [], []
    ...
    while last_s is None or last_s < s_stop:
        ...
        x_list.append(float(meas["x"]))
        y_list.append(float(meas["y"]))
...
for case_num, data in case_data.items():
    ax.plot(data["x"], data["y"], label=f"Case {case_num}", color=colors[case_num])

# Optionally overlay the reference centerline
ax.plot(reference.x, reference.y, "k--", linewidth=1, label="Reference")

ax.set_xlabel("X [m]")
ax.set_ylabel("Y [m]")
ax.set_title("Cartesian Trajectories — Case Comparison")
ax.legend()
ax.set_aspect("equal")
ax.grid(True)
plt.tight_layout()
...
colors = {1: "tab:blue", 2: "tab:orange"}
for case_num, data in case_data.items():
    ax.plot(data["s"], data["e"], label=f"Case {case_num}", color=colors[case_num])
...
colors = {1: "tab:blue", 2: "tab:orange"}
for case_num, data in case_data.items():
    ax.plot(data["s"], data["dphi"], label=f"Case {case_num}", color=colors[case_num])
ax.set_xlabel("path distance [m]")
...
colors = {1: "tab:blue", 2: "tab:orange"}
for case_num, data in case_data.items():
    ax.plot(data["s"], data["V"], label=f"Case {case_num}", color=colors[case_num])
ax.set_xlabel("path distance [m]")
...
colors = {1: "tab:blue", 2: "tab:orange"}
for case_num, data in case_data.items():
    ax.plot(data["s"], data["delta"], label=f"Case {case_num}", color=colors[case_num])
ax.set_xlabel("path distance [m]")
...
colors = {1: "tab:blue", 2: "tab:orange"}
for case_num, data in case_data.items():
    ax.plot(data["s"], data["torque"], label=f"Case {case_num}", color=colors[case_num])
ax.set_xlabel("path distance [m]")
