...
class FialaBicycleCasADi:
    ...
    def _compute_wheel_torque_terms(self, state_control: Dict[str, Any]) -> Dict[str, Any]:
        """Data-driven rear-wheel torque surrogate fitted from BeamNG data."""
        ...
        wr_scaled   = data_scaled["rear_wheelspeed_ms"]
        gr_scaled   = data_scaled["gr"]

        # Hidden layer — tanh activations
        # W1 shape (6, 5), inputs: [eng_s, boost_s, throttle, wr_s, gr]
        hidden_1 = ca.tanh(
            (0.11167208 * eng_scaled) + (-0.40413949 * boost_scaled) +
            (1.01859331 * throttle)   + (1.43400550 * wr_scaled) +
            (-2.35317421 * gr_scaled) + (-3.24455190)
        )
        hidden_2 = ca.tanh(
            (2.84211707 * eng_scaled) + (-0.37464678 * boost_scaled) +
            (2.62670970 * throttle)   + (2.66615605 * wr_scaled) +
            (2.56018376 * gr_scaled)  + (2.55319428)
        )
        hidden_3 = ca.tanh(
            (-0.14319304 * eng_scaled) + (0.22590195 * boost_scaled) +
            (0.79743034 * throttle)    + (-3.61168933 * wr_scaled) +
            (3.60289168 * gr_scaled)   + (-0.35119569)
        )
        hidden_4 = ca.tanh(
            (-1.13340831 * eng_scaled) + (-0.59167457 * boost_scaled) +
            (1.39741814 * throttle)    + (0.58558375 * wr_scaled) +
            (-0.66802365 * gr_scaled)  + (-0.86888808)
        )
        hidden_5 = ca.tanh(
            (0.57002389 * eng_scaled) + (-0.66202265 * boost_scaled) +
            (-4.83261919 * throttle)  + (0.81010717 * wr_scaled) +
            (-2.32801819 * gr_scaled) + (-0.08349965)
        )
        hidden_6 = ca.tanh(
            (1.17721498 * eng_scaled) + (0.50811827 * boost_scaled) +
            (-2.34793711 * throttle)  + (0.06239882 * wr_scaled) +
            (0.19668479 * gr_scaled)  + (-0.74856281)
        )

        # Output layer — linear, scaled back to Nm
        # W2 shape (1, 6)
        total_torque = 1000.0 * (
            (-1.30979133 * hidden_1) +
            (0.42479926  * hidden_2) +
    ...
    def estimate_inverse_throttle(self, state_control: Dict[str, Any], desired_torque):
        """Data-driven inverse-throttle surrogate fitted from BeamNG data."""
        engine_speed_rads  = state_control["engine_speed_rads"]
        boost_pressure     = state_control["boost_pressure"]
        rear_wheelspeed_ms = state_control["rear_wheelspeed_ms"]
        ...
        gear_ratio_scaled     = 0.01 * (
            engine_speed_rads / ((rear_wheelspeed_ms / wheel_radius) + 1e-4)
        )
        zero = 0.0

        # Hidden layer — ReLU activations
        # W1 shape (4, 4), inputs: [eng_s, boost_s, T_des_s, gr]
        hidden_1 = ca.fmax(
            (0.16410351  * engine_speed_scaled) +
            (0.40552694  * boost_scaled)        +
            (0.62166083  * desired_torque_scaled) +
            (-5.05002165 * gear_ratio_scaled)   +
            (-0.11608534),
            zero
        )
        hidden_2 = ca.fmax(
            (-0.81771392 * engine_speed_scaled) +
            (0.41312507  * boost_scaled)        +
            (1.04903293  * desired_torque_scaled) +
            (2.05346704  * gear_ratio_scaled)   +
            (1.02183521),
            zero
        )
        hidden_3 = ca.fmax(
            (0.37518087  * engine_speed_scaled) +
            (-0.80569553 * boost_scaled)        +
            (-1.41657174 * desired_torque_scaled) +
            (3.15969801  * gear_ratio_scaled)   +
            (-0.45527574),
            zero
        )
        hidden_4 = ca.fmax(
            (0.12647820  * engine_speed_scaled) +
            (-0.83477741 * boost_scaled)        +
            (0.61374289  * desired_torque_scaled) +
            (-5.05048132 * gear_ratio_scaled)   +
            (-0.03851702),
            zero
        )

        # Output layer — sigmoid to constrain to (0, 1)
        # W2 shape (1, 4)
        logit = (
            (2.86614561  * hidden_1) +
            (-3.27134871 * hidden_2) +
    ...
    def build_powertrain_functions_from_data(self, casadi_prefix: str):
        """
        Load empirically fitted torque/inverse maps and wrap them to match
        the interface of build_powertrain_functions().

        The fitted maps use (delta, omega_rad_s) where omega = rear wheel
        angular velocity in rad/s. Engine speed and boost are ignored since
        the data-driven map already captures their effect implicitly.
        """

        torque_fn       = ca.Function.load(f"{casadi_prefix}_casadi.casadi")
        inv_throttle_fn = ca.Function.load(f"{casadi_prefix}_inv_casadi.casadi")
        wheel_radius    = float(self.wheel["wheel_radius"])

        eng   = ca.SX.sym("eng")
        boost = ca.SX.sym("boost")
        thr   = ca.SX.sym("throttle")
        wr_ms = ca.SX.sym("wr_ms")
        t_des = ca.SX.sym("T_des")

        omega = wr_ms / wheel_radius  # m/s → rad/s

        return {
            "torque_est_fn": ca.Function(
                "torque_est",
                [eng, boost, thr, wr_ms],
                [torque_fn(thr, omega, eng, boost)],  # now uses all 4 inputs
            ),
            "inv_throttle_fn": ca.Function(
                "inv_torque",
                [eng, boost, wr_ms, t_des],
                [inv_throttle_fn(t_des, omega, eng, boost)],
            ),
        }
